...
import { auth, plates } from '~/api/client';
...
describe('API Client', () => {
  ...
  describe('network errors', () => {
    it('throws user-friendly message when backend is down', async () => {
      ...
      await expect(plates.collect('aB3x', 'KA NR 355')).rejects.toThrow('Could not reach the server. Is the backend running?');
    });
    ...
    it('throws user-friendly message on network timeout', async () => {
      ...
      await expect(plates.collect('aB3x', 'KA NR 355')).rejects.toThrow('Could not reach the server');
    });
  });

  describe('server errors', () => {
    it('extracts error message from JSON response', async () => {
      vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({ error: 'User not found' }), {
  ...
  describe('successful requests', () => {
    it('plates.collect sends correct payload', async () => {
      const mockFetch = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
        plate: { id: 1, userId: 'aB3x', plateText: 'KA NR 355', collectedAt: '2026-05-15' }, challengeUpdates: [],
      }), {
