...
describe('PlateSubmission — 2-col layout', () => {
  ...
  it('preview shows region badge after successful lookup', async () => {
    col('kennzeichen').getFirstListItem.mockResolvedValueOnce({
      id: 'kz1', code: 'M', district_name: 'München', bundesland: 'Bayern',
...
describe('PlateSubmission — map pan on plate input', () => {
  ...
  it('pans to geo_region centroid for M', async () => {
    col('kennzeichen').getFirstListItem.mockResolvedValueOnce({
      id: 'kz-m', code: 'M', district_name: 'München', bundesland: 'Bayern',
      bundesland_iso: 'DE-BY', derivation: 'München', active: true, notes: '',
    });
    ...
    typeInPlate('M AB 1234');
    await waitFor(() => {
      expect(mockFlyToCoords).toHaveBeenCalledWith(
        [11.58, 48.13],
        9,
        expect.any(Array),
      );
  ...
  it('pans to geo_region centroid for B (Berlin)', async () => {
    col('kennzeichen').getFirstListItem.mockResolvedValueOnce({
      id: 'kz-b', code: 'B', district_name: 'Berlin', bundesland: 'Berlin',
      bundesland_iso: 'DE-BE', derivation: 'Berlin', active: true, notes: '',
    });
  ...
  it('pans to actual Karlsruhe centroid for KA (not Stuttgart)', async () => {
    col('kennzeichen').getFirstListItem.mockResolvedValueOnce({
      id: 'kz-ka', code: 'KA', district_name: 'Karlsruhe', bundesland: 'Baden-Württemberg',
      bundesland_iso: 'DE-BW', derivation: 'KArlsruhe', active: true, notes: '',
    });
    ...
    await waitFor(() => {
      ...
      expect(mockFlyToCoords).not.toHaveBeenCalledWith(
        [9.182, 48.776],
        expect.anything(),
        expect.anything(),
      );
