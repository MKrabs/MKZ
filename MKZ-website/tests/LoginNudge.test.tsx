...
describe('LoginNudge', () => {
  ...
  it('SVG arrow path uses stroke-dasharray for draw animation', () => {
    ...
    const hasDrawEffect = animatedPaths.some(
      (p) =>
        p.classList.contains('draw-arrow') ||
  ...
  it('SVG arrow has an arrowhead (marker-end or polygon)', () => {
    ...
    const svg = screen.getByTestId('login-nudge-arrow');
    const hasMarker =
      !!svg.querySelector('marker') ||
      !!svg.querySelector('polygon') ||
      Array.from(svg.querySelectorAll('path')).some(
        (p) =>
          p.getAttribute('marker-end') !== null || p.getAttribute('markerEnd') !== null,
      );
  ...
  it('text nudge box is also position:fixed (not confined inside a card)', () => {
    ...
    const isFixed =
      ...
      nudge.classList.contains('fixed');
    expect(isFixed).toBe(true);
  });
  ...
  it('arrowhead polygon has appear-after-draw class so it is hidden until the path is complete', () => {
    ...
    const hasClass =
      polygon!.classList.contains('appear-after-draw') ||
      (group !== null && group.classList.contains('appear-after-draw'));
