...
const ChallengeExpanded: Component<ChallengeExpandedProps> = (props) => {
  return (
    <div class="space-y-4" data-testid="challenge-expanded">
      {props.challenge.progress.type === 'regions' && (
        <RegionsDetail progress={props.challenge.progress} />
      )}
      {props.challenge.progress.type === 'highscore' && (
        <HighscoreDetail challenge={props.challenge} />
      )}
      {props.challenge.progress.type === 'composite' && (
        <CompositeDetail progress={props.challenge.progress} />
      )}
...
const RegionsDetail: Component<{ progress: Extract<import('./ChallengesGrid').ChallengeProgress, { type: 'regions' }> }> = (props) => {
  // German state codes to names
  const REGIONS: Record<string, string> = {
    'BY': 'Bayern', 'BW': 'Baden-Württemberg', 'NRW': 'Nordrhein-Westfalen',
    'HE': 'Hessen', 'NI': 'Niedersachsen', 'SH': 'Schleswig-Holstein',
    'HH': 'Hamburg', 'HB': 'Bremen', 'BE': 'Berlin', 'BB': 'Brandenburg',
        ...
        <div class="text-center">
          <div class="grid grid-cols-4 gap-2 mb-4">
            <For each={Object.entries(REGIONS)}>
              {([code, name]) => (
                <div
                  class={`px-2 py-1 rounded text-xs font-medium text-center transition-colors ${
                    props.progress.foundRegions.includes(code)
                      ? 'bg-mkz-primary text-white'
...
const HighscoreDetail: Component<{ challenge: ChallengePreview }> = (props) => {
  ...
  const entries = [
    { rank: 1, name: 'Bob', plate: 'B X 1', score: 200 },
    { rank: 2, name: 'Max', plate: 'M G 12', score: 135 },
...
const CompositeDetail: Component<{ progress: Extract<import('./ChallengesGrid').ChallengeProgress, { type: 'composite' }> }> = (props) => {
  return (
    <div class="space-y-2">
          ...
          {(item) => (
            <div class="flex items-center gap-3 p-2 rounded-md bg-gray-50">
              <div class={`w-5 h-5 rounded-full flex items-center justify-center ${
                item.found ? 'bg-mkz-primary' : 'bg-gray-200'
              }`}>
