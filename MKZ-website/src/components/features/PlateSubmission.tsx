...
const NO_PHOTO_MSGS = [
  ...
  '📋 Logged, but the jury needs proof!',
  "🫣 You say you saw it... sure, buddy.",
  '📝 Noted, but evidence is pending.',
  '🤔 Trust me bro? Snap a pic!',
  '🎩 On file — now make it official.',
  '📵 Words without a photo = just a rumour.',
  '👀 Spotted? Prove it!',
];

const HAS_PHOTO_MSGS = [
  '📸 Full documentation — legend!',
  '🏅 Caught on camera!',
      ...
      <div class="bg-white/40 backdrop-blur-sm rounded-xl shadow-lg border border-white/40 p-6">
        ...
        <div class="flex flex-col gap-4">
          ...
          <Show when={kennzeichen()}>
            <div class="flex items-start gap-3 bg-blue-50/80 rounded-lg px-4 py-3 border border-blue-100/60" data-testid="region-info">
                ...
                <Show
                  ...
                  fallback={
                    <label class="cursor-pointer inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors text-sm text-gray-600 w-fit">
                      ...
                      <input ref={imageInputRef} type="file" accept="image/*" class="hidden" onChange={handlePendingImageSelect} data-testid="image-upload-input" />
                    </label>
                  }
                >
                  <div class="flex items-center gap-3">
                    <img src={pendingImageUrl()!} alt="Preview" class="h-14 w-20 object-cover rounded-lg border border-gray-200" />
                    <div class="flex flex-col gap-1">
                      <span class="text-sm text-gray-600 font-medium">Photo ready</span>
                      <button type="button" onClick={clearPendingImage} class="text-xs text-red-500 hover:underline w-fit">Remove</button>
                    </div>
                  </div>
                </Show>
                <div class="flex items-center gap-3 flex-wrap">
                  <Button onClick={handleMarkSeen} loading={submitting()} disabled={submitting()} testId="mark-seen-btn">
                    Mark as Found!
                  </Button>
                </div>
              </Show>
                ...
                <div class="space-y-3" data-testid="already-seen-row">
                  <Show when={!seenRecord()!.image}>
                    <div class="bg-amber-50/80 border border-amber-200/60 rounded-lg px-4 py-3">
                      <p class="text-sm font-medium text-amber-800" data-testid="already-seen-msg">{pickMsg(NO_PHOTO_MSGS, plateText())}</p>
                      <p class="text-xs text-amber-600 mt-1">Logged <span class="font-mono">{seenRecord()!.plate_text}</span> — upload evidence to seal the deal.</p>
                    </div>
                    <div class="flex items-center gap-3 flex-wrap">
                      <label class="cursor-pointer inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-mkz-primary text-white text-sm font-medium hover:bg-mkz-secondary transition-colors">
                        ...
                        <input ref={updateImageInputRef} type="file" accept="image/*" class="hidden" onChange={handleAddPhoto} data-testid="add-photo-input" />
                      </label>
                      <Button variant="ghost" onClick={handleRemoveEntry} loading={submitting()} disabled={submitting()} testId="remove-seen-btn">
                        Remove from list
                      </Button>
                    </div>
                  </Show>
                  ...
                  <Show when={seenRecord()!.image}>
                    <div class="bg-emerald-50/80 border border-emerald-200/60 rounded-lg px-4 py-3">
                      <p class="text-sm font-medium text-emerald-800" data-testid="already-seen-msg">{pickMsg(HAS_PHOTO_MSGS, plateText())}</p>
                      <p class="text-xs text-emerald-600 mt-1"><span class="font-mono">{seenRecord()!.plate_text}</span> — documented with proof.</p>
                    </div>
                    <div class="flex items-start gap-4">
                      <img src={getSeenImageUrl(seenRecord()!, '200x200')} alt="Plate photo" class="h-24 w-32 object-cover rounded-xl border border-gray-200 shadow-sm" data-testid="seen-photo" />
                      <div class="flex flex-col gap-2">
                        <label class="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 text-sm text-gray-600 transition-colors">
                          ...
                          <input ref={updateImageInputRef} type="file" accept="image/*" class="hidden" onChange={handleAddPhoto} data-testid="change-photo-input" />
                        </label>
                        <button type="button" onClick={handleRemovePhoto} disabled={submitting()} class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 text-sm transition-colors disabled:opacity-50" data-testid="remove-photo-btn">
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                          </svg>
                          Remove photo
                        </button>
                        <button type="button" onClick={handleRemoveEntry} disabled={submitting()} class="text-xs text-gray-400 hover:text-red-500 hover:underline text-left transition-colors" data-testid="remove-seen-btn">
                          Remove entire entry
                        </button>
                      </div>
                    </div>
