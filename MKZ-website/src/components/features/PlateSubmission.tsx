...
/**
 * Shown when the plate is on record but has NO photo.
 * Chosen deterministically from plate_text so it's stable per plate.
 */
const NO_PHOTO_MSGS = [
...
/** Shown when the plate is on record AND has a photo. */
...
const PlateSubmission: Component = () => {
  ...
  // Dedup tracking — avoid redundant network calls
...
// Replace it with this single onMount listener:
  ...
  const handleMarkSeen = async () => {
    ...
    try {
      ...
      if (msg.includes('unique') || msg.includes('constraint')) {
        ...
        const existing = await checkSeen(u.id, text);
        setSeenRecord(existing);
      } else {
  ...
  const handleAddPhoto = async (e: Event) => {
    ...
    setSubmitError(null);
    try {
      const updated = await updateSeenImage(rec.id, file);
      setSeenRecord(updated);
    } catch {
  ...
  const handleRemovePhoto = async () => {
    ...
    setSubmitError(null);
    try {
      const updated = await removeSeenImage(rec.id, rec.image);
      setSeenRecord(updated);
    } catch {
  ...
  const regionCalloutData = createMemo<RegionData | null>(() => {
    ...
    return {
      ...
      plateCount: 0, // placeholder — will be populated later
      funFacts: [
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Render
  // Outer div = ONLY the grid — no background, so col-span-4 preview is
  // transparent down to the fixed map layer.
  // ─────────────────────────────────────────────────────────────────────────
  return (
    <div class="grid md:grid-cols-2 gap-4 lg:mt-48 md:mt-32" data-testid="plate-submission">
      ...
      <div class="bg-white/40 backdrop-blur-sm rounded-xl shadow-lg border border-white/40 p-6">
        ...
        <div class="flex flex-col gap-4">
          ...
          {/* ════════════════════════════════════════════════════════════════
              Action zone — rendered only when the region is known
              ════════════════════════════════════════════════════════════════ */}
          <Show when={kennzeichen()}>
            ...
            <Show when={user()}>
              ...
              {/* ── STATE: NOT YET LOGGED ─────────────────────────────── */}
              <Show when={!seenRecord()}>
                {/* Optional photo before first save */}
                <Show
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-1">
                      ...
                      <button
                        ...
                        class="text-xs text-red-500 hover:underline w-fit"
                      >
                        Remove
                      </button>
                    </div>
                ...
                <div class="space-y-3" data-testid="already-seen-row">
                  ...
                  <Show when={!seenRecord()!.image}>
                    <div class="bg-amber-50/80 border border-amber-200/60 rounded-lg px-4 py-3">
                      <p class="text-sm font-medium text-amber-800" data-testid="already-seen-msg">
                  ...
                  <Show when={seenRecord()!.image}>
                    <div class="bg-emerald-50/80 border border-emerald-200/60 rounded-lg px-4 py-3">
                      <p class="text-sm font-medium text-emerald-800" data-testid="already-seen-msg">
                        {pickMsg(HAS_PHOTO_MSGS, plateText())}
                      </p>
                      <p class="text-xs text-emerald-600 mt-1">
                        <span class="font-mono">{seenRecord()!.plate_text}</span> — documented with proof.
                      </p>
                    </div>
                    ...
                    <div class="flex items-start gap-4">
                      <img
                        src={getSeenImageUrl(seenRecord()!, '200x200')}
                        alt="Plate photo"
          ...
          </Show>{/* end: kennzeichen found */}

        </div>
      </div>

      {/* col-span-4 — transparent preview window */}
      <div
        ref={previewRef}
        data-testid="map-preview-box"
        class="relative rounded-xl overflow-hidden min-h-45"
      >
        {/* Viewfinder corners */}
